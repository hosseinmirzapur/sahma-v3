...
class OfficeService
{
  ...
  public function generateWindowsEntityGroup(EntityGroup $entityGroup): array
  {
    ...
    foreach ($entityGroup->entities as $entity) {
      // Ensure meta is an array and has csv_location
      if (!is_array($entity->meta) || empty($entity->meta['csv_location']) || !Storage::disk('csv')->exists($entity->meta['csv_location'])) {
        Log::warning("OfficeService => generateWindowsEntityGroup: Skipping entity #{$entity->id} due to missing/invalid CSV location or meta structure.");
        continue; // Skip this entity
      }

      // Safely access nested 'window' and 'start' keys
      $beginningOfWindow = 0;
      if (isset($entity->meta['window']) && is_array($entity->meta['window']) && isset($entity->meta['window']['start'])) {
        $beginningOfWindow = $entity->meta['window']['start'];
      } else {
        Log::warning("OfficeService => generateWindowsEntityGroup: 'window' or 'start' key missing/invalid in meta for entity #{$entity->id}. Defaulting beginningOfWindow to 0.");
      }
