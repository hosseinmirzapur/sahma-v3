<template>
  <LayoutSideBar>
    <!-- dropDown btn -->
    <div
      class="relative w-full bg-primary text-white rounded-xl text-xl shadow-btnUni
        overflow-hidden h-12 min-h-[3rem] bg-gradient-to-br from-white to-white
        transition-all ease-in-out delay-150 mb-5"
    ...
    <Folder
      ...
      :is-modal="false" />
    <div
      class="flex absolute bottom-0 pb-4 bg-gradient-to-t via-primary/80 from-primary
                w-[85%] justify-center gap-x-5">
      <div
        ...
        <ArchiveBoxIcon
          ...
          :class="{' pointer-events-none opacity-50': authUser?.isUser||authUser?.isReadOnly}" />
  ...
  <Modal
    ...
    <form
      class="w-full"
      @submit.prevent="subCreateFolder">
      <div
        ...
        <button
          class="flex-shrink-0 bg-primary hover:bg-primaryDark border-primary hover:border-primaryDark
                 text-sm border-4 text-white py-1 px-2 rounded  disabled:opacity-50 disabled:cursor-not-allowed"
          type="submit"
      ...
      <form @submit.prevent="subUploadFile">
        ...
        <div class="flex items-center justify-center mt-3">
          <label
            ...
            :class="{'border-red-600' : errorsFile.includes('files') , 'border-green-600 bg-green-600/5':isDragover}"
            @drop="drop"
            ...
            <div class="flex flex-col items-center justify-center w-full">
              <template v-if="!formFile?.file?.name">
                <PlusCircleIcon
                  ...
                  :class="{'!text-red-600' : errorsFile.includes('files'),'!text-green-600 ':isDragover}" />
              </template>
              <template v-else>
                <button
                  type="button"
...
import { ArchiveBoxIcon, PlusCircleIcon, TrashIcon } from '@heroicons/vue/24/outline'
...
const format = reactive({
  text: 'فایل باید از نوع wav, mp3, aac, flac, wma, ogg, m4a, pdf, wma, mp4, jpeg, word, tif باشد.',
  rex: /(\.wav|\.mp3|\.pdf|\.aac|\.flac|\.wma|\.jpg|\.ogg|\.mp4|\.jpeg|\.tif|\.docx|\.doc)$/i
})
...
const getErrorText = (error) => {
  switch (error) {
    ...
    case 'rex':
      return 'فایل باید از نوع wav, mp3, aac, flac, wma, ogg, m4a, pdf, wma, mp4, jpeg, tiff باشد.'
    default:
      return error
  }
}
...
watch(
  ...
  () => {
    errorsFile.value = errorsFile.value.filter(v => !['files', 'rex'].includes(v))
  }
)
watch(
...
function validCreateFolder () {
  errors.value = []
  return !form.folderName ? errors.value.push('نام فایل را مشخص کنید .') : false
}
...
function subCreateFolder () {
  ...
  const choiceRoute = getSlugFromUrl() ? 'web.user.dashboard.folder.create' : 'web.user.dashboard.folder.create-root'
  // eslint-disable-next-line no-undef
...
function subUploadFile () {
  ...
  setAlerts(false, 'success', '', []) // restart Alert
  const choiceRoute = getSlugFromUrl() ? 'web.user.dashboard.file.upload' : 'web.user.dashboard.file.create-root'
