\<template>
  <header class="fixed !w-[calc(100vw-3.5rem)] m-auto top-5 right-0 left-0 bottom-auto w-full bg-white">
    <div class="w-full flex justify-between items-center md:h-20 h-16 p-5 rounded-xl shadow-cardUni bg-primary">
      ...
      <div
        ...
        <input
          ...
          class="placeholder-gray-400 bg-transparent border-x-0 border-t-0 border-b-1 !border-gray-300 text-gray-200 w-full py-2 px-4 focus:outline-none focus:bg-transparent
      ...
      <div class="w-full flex justify-end">
        ...
        <div
          ...
          <div
            ...
            <button
              ...
              :class="[conditionItems(item.name), {'hidden':item.name!=='back' && authUser?.isUser}]"
            ...
            <transition name="tooltip">
              <div
                ...
                class="absolute w-max top-20 w-full left-0 z-10 p-5 text-sm font-medium text-primary transition-opacity duration-300 bg-white rounded-md tooltip">
        ...
        <div
          ...
          <div
            v-for="(item, i) in options.slice(-3)"
            :key="i"
            class="relative">
            <button
              type="button"
        ...
        <ul
          ...
          <li>
            <button
              ...
              :class="{'hidden': file?.status!=='TRANSCRIBED','hidden' : fileType !=='pdf'}"
      ...
      <Modal
        ...
        <ol class="relative border-r border-gray-700 mt-5 w-full ">
          <li
            ...
            <div class="absolute w-3 h-3 bg-gray-900 rounded-full mt-1.5 -right-1.5 border border-gray-900 z-10" />
            <time
        ...
        <div class="flex justify-center gap-x-5 mt-5">
          <button
            class="flex-shrink-0 bg-primary hover:bg-primaryDark hover:border-primaryDark text-sm
              text-white w-36 py-2 px-4 rounded-xl hidden"
            type="submit"
            v-text="`پرینت`" />
          <button
...
function tryAgain (file) {
  Inertia.visit(route('web.user.dashboard.file.transcribe', { fileId: file?.slug }), {
    method: 'post',
    replace: true,
    preserveState: true
    // onError: (e) => {
...
const backToFolder = () => {
  Inertia.visit(props.searchedInput ? route('web.user.dashboard.search-form') : props.parentFolder, {
    method: 'get',
