  ...
  <div class="p-5 min-h-[calc(100vh-9rem)]">
    ...
    <ul
      v-if="
        ...
        (searchFolder.length > 0 || searchFiles.length > 0)
      "
    ...
    >
        ...
        >
          ...
          >
            <ul class="py-2 text-sm text-gray-800">
              <li v-for="(item, j) in options" :key="j">
                <button
                  :id="item.slug"
                  class="block px-4 py-2 w-full hover:bg-gray-100 text-center"
                  :class="{
                    hidden: item.slug !== 'open' && authUser?.isUser,
                    'disabled:cursor-not-allowed disabled:opacity-50 hidden':
                      checkMultiSelectItem(item),
                  }"
      ...
      <template v-if="files?.length > 0">
        ...
        >
          ...
          >
            <ul class="py-2 text-sm text-gray-800">
              <li v-for="(item, j) in optionsFile" :key="j">
                <button
        ...
        <form class="w-full" @submit.prevent="subRename">
          ...
          >
            ...
            <button
              ...
              type="submit"
              v-text="
                currFile.length > 0 || currFolder.length > 0
                  ? `ویرایش`
                  : 'ایجاد'
              "
            />
      ...
      >
        ...
        <div class="flex justify-center gap-x-6">
          <button
            class="flex-shrink-0 bg-primary hover:bg-primaryDark border-primary text-sm border shadow-btnUni text-white w-36 py-2 px-4 rounded rounded-xl cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      >
        <div class="flex flex-col items-start relative">
          ...
          <div
            id="dropdown"
            class="flex flex-row justify-start gap-x-2 items-center mt-3 rounded-lg border border-gray-300 w-full h-[2.5rem] py-2 px-3 focus:outline-none focus:border-none focus:ring-0 relative"
          ...
          >
            ...
            >
              <span>{{
                departments.find((v) => v.id === item.id)?.name + "  "
              }}</span>
            </p>
        ...
        <div class="flex justify-center">
          <button
            class="flex-shrink-0 bg-primary hover:bg-primaryDark border-primary hover:border-primaryDark shadow-btnUni text-sm border-4 text-white w-full py-2 px-4 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed mt-3"
    ...
    <!-- empty page-->
    <div v-else class="h-[calc(100vh-20rem)] flex justify-center item center">
      <div
        class="w-1/2 rounded-xl flex flex-col items-center gap-y-8 text-lg text-base p-5 m-auto"
      >
        <svg
...
function checkedAllItems() {
  if (isCheckBoxing.value) {
    ...
    const areEqFolder =
      form.folders.length === foldersId.length &&
      form.folders.every((value, index) => value === foldersId[index]);
    const areEqFiles =
      form.folders.length === foldersId.length &&
      form.folders.every((value, index) => value === foldersId[index]);
...
const openDepChange = () => {
  ...
  if (
    currFile.value[0]?.departments &&
    currFile.value[0]?.departments.length > 0
  ) {
...
const searchFolder = computed(() => {
  return textSearch.value
    ? sortFoldersItem.value.filter((item) =>
        item.name.includes(textSearch.value),
      )
    : sortFoldersItem.value;
});

const searchFiles = computed(() => {
  return textSearch.value
    ? sortFilesItem.value.filter((item) => item.name.includes(textSearch.value))
    : sortFilesItem.value;
});
...
function setBgFiles(type) {
  switch (type) {
    ...
    case "spreadsheet": // Added case for spreadsheet
      return "bg-green-100/50 text-green-700"; // Example style
    default:
      return type;
  }
}
